...
test('Scraper can get metrics', async (t) => {
  ...
  dhtPromClient.on('metrics-request', ({ uid, remotePublicKey }) => {
    reqUid = uid
    t.alike(remotePublicKey, scraperPubKey, 'metrics-request emitted')
  })
  ...
  t.is(metrics.includes('process_cpu_system_seconds_total'), true, 'Got prometheus metrics')
  ...
  t.ok(
    infoMessages[0].includes('Prom client attempting to register dummy-alias'),
    'alias-attempt log'
  )
  ...
  t.ok(
    debugMessages[0].includes('Prom client received metrics request from'),
    'metric req received log'
  )
  t.ok(
    debugMessages[1].includes('Prom client successfully processed metrics request'),
    'metric req processed log'
  )
})
...
test('Can pass own getMetrics', async (t) => {
  ...
  const protomuxRpcClient = new ProtomuxRpcClient(dht, {
    requestTimeout: 1000,
    backoffValues: [100, 250, 500]
  })

  const { scraperProtomuxRpcClient, scraperPubKey } = setupscraperRpcClient(t, bootstrap)
  ...
  const metrics = res.metrics
  t.is(metrics === 'Some metrics', true, 'Got prometheus metrics')
...
test('Scraper can timeout', async (t) => {
  ...
  await t.exception(
    () => scraper.requestMetrics({ timeout: 50 }),
    /TIMEOUT_EXCEEDED/,
    'Timeout error'
  )
...
test('Other clients cannot get metrics', async (t) => {
  ...
  const { scraperProtomuxRpcClient, scraperPubKey: otherPubKey } = setupscraperRpcClient(
    t,
    bootstrap
  )

  dhtPromClient.on('firewall-block', ({ remotePublicKey }) => {
    t.alike(remotePublicKey, otherPubKey, 'Firewall blocked the request')
  })

  const scraper = await setupScraper(t, scraperProtomuxRpcClient, dhtPromClient)

  await t.exception(() => scraper.requestMetrics(), /REQUEST_TIMEOUT/, 'Client cannot connect')

  t.ok(
    infoMessages[1].includes('Firewall blocked unauthorised connection attempt from'),
    'firewall-block log'
  )
})
...
test('client regularly re-registers itself', async (t) => {
  ...
  dhtPromClient.aliasClient.on('alias-attempt', ({ alias, targetKey, hostname, service }) => {
...
test('Error handling when getting metrics throws', async (t) => {
  ...
  t.ok(
    infoMessages[2].includes('Prom client failed to process metrics request'),
    'metrics-error log'
  )
})
...
async function setup(t, clientOpts = {}) {
  ...
  const scraperProtomuxRpcClient = new ProtomuxRpcClient(scraperDht, {
    requestTimeout: 1000,
    backoffValues: [100, 250, 500]
  })
...
function setupscraperRpcClient(t, bootstrap) {
  const scraperDht = new HyperDHT({ bootstrap })
  const scraperProtomuxRpcClient = new ProtomuxRpcClient(scraperDht, {
    requestTimeout: 1000,
    backoffValues: [100, 250, 500]
